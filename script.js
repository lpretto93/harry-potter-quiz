...
// Funzione per caricare le domande dal file JSON ospitato su GitHub Pages
async function loadQuestions() {
    try {
        const response = await fetch('https://username.github.io/repository-name/questions.json'); // Usa l'URL completo del JSON su GitHub Pages
        if (!response.ok) {
...
function checkAnswer(selectedIndex) {
    ...
    if (currentQuestionIndex < questions.length) {
        ...
        setTimeout(() => {
            showLeaderboard(); // Mostra la classifica se il quiz è finito
        }, 3000);
    }
}

// Mostra la classifica alla fine del quiz
function showLeaderboard() {
    ...
    nextButton.style.display = 'none'; // Nasconde il bottone "Gioca ancora"
}
...
// Gestisce il click sul tasto "Gioca ancora"
nextButton.addEventListener('click', () => {
    leaderboardModal.style.display = 'none'; // Nasconde il modale
    score = 0; // Resetta il punteggio
    currentQuestionIndex = 0; // Resetta l'indice delle domande

    showQuestion(); // Mostra una nuova partita
});
